var OverPassLayer = L.FeatureGroup.extend({
    ...
    options: {
        ...
        onSuccess: function (data) {
            for (var i = 0; i < data.elements.length; i++) {
                ...
                popupContent = this._getPoiPopupHTML('way', e.tags, e.id);
                popup = L.popup().setContent(popupContent);
                marker.bindPopup(popup);

                this.options.childLayers['childLayer'].addLayer(marker);
            }
        },
        ...
        minZoomIndicatorOptions: {
            ...
            //'minZoomMessage': 'Zoomniveau: CURRENTZOOM. Data laadt bij zoomniveau: MINZOOMLEVEL.',
            'minZoomMessage': 'Zoom in om DATANAMES te laden.',
        },
    },
    ...
    _getPoiPopupHTML: function (type, tags, id) {
        ...
        link.href = 'http://www.openstreetmap.org/edit?editor=id&' + type + '=' + id;
    ...
    _buildLargerBounds: function (bounds) {
        ...
        //
        bounds._southWest.lat -= biggestDimension / 4;
        bounds._southWest.lng -= biggestDimension / 4;
        bounds._northEast.lat += biggestDimension / 4;
        bounds._northEast.lng += biggestDimension / 4;
    ...
    _prepareRequest: function () {
        ...
        if (this._map.getZoom() < this.options.minZoom) { //|| !this._isVisible
            return false;
        }
    ...
    onAdd: function (map) {
        ...
        for (var name in this.options.childLayers) {
            if (this.options.markerCluster !== null)
                this.options.childLayers[name] = L.featureGroup.subGroup(this.options.markerCluster).addTo(this._map);
            else
                this.options.childLayers[name] = L.featureGroup().addTo(this._map);
        }
    ...
    enableZoomControl: function (visibility) {

        if (visibility) {
            if (!(this._zoomControl._hasLayer(this)))
                this._zoomControl._addLayer(this);
        } else {
            if ((this._zoomControl._hasLayer(this)))
