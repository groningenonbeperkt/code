...
namespace Microsoft.AspNetCore.Authentication.Openstreetmap
{
    internal class OpenstreetmapHandler : RemoteAuthenticationHandler<OpenstreetmapOptions>
    {
        ...
        /// </summary>
        protected new OpenstreetmapEvents Events
        {
            get { return (OpenstreetmapEvents)base.Events; }
            set { base.Events = value; }
        }

        public OpenstreetmapHandler(IOptionsMonitor<OpenstreetmapOptions> options, ILoggerFactory logger, UrlEncoder encoder, ISystemClock clock)
            : base(options, logger, encoder, clock)
        { }

        protected override Task<object> CreateEventsAsync() => Task.FromResult<object>(new OpenstreetmapEvents());
        ...
        protected override async Task<HandleRequestResult> HandleRemoteAuthenticateAsync()
        {
            ...
            {
                new Claim(OpenstreetmapClaimTypes.AccessToken, accessToken.Token, ClaimValueTypes.String, ClaimsIssuer),
                new Claim(OpenstreetmapClaimTypes.AccessTokenSecret, accessToken.TokenSecret, ClaimValueTypes.String, ClaimsIssuer)
            },
            ClaimsIssuer);
            ...
            ClaimsIdentity identity, AuthenticationProperties properties, AccessToken token, JObject user)
        {
            ...
            var context = new OpenstreetmapCreatingTicketContext(Context, Scheme, Options, new ClaimsPrincipal(identity), properties, token.UserId, token.ScreenName, token.Token, token.TokenSecret, user);
        ...
        protected override async Task HandleChallengeAsync(AuthenticationProperties properties)
        {
            ...
            var redirectContext = new RedirectContext<OpenstreetmapOptions>(Context, Scheme, Options, properties, OpenstreetmapAuthenticationEndpoint);
        ...
        private async Task<RequestToken> ObtainRequestTokenAsync(string callBackUri, AuthenticationProperties properties)
        {
            ...
            {
                throw new Exception("Openstreetmap oauth_callback_confirmed is not true.");
            }
